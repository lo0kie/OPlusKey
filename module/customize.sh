...
# 配置目录在模块目录之外，更新模块时不会被清掉
mkdir -p /data/adb/OPlusKey

chmod 0755 "$MODDIR" "$MODDIR/bin" "$MODDIR/webroot" 2>/dev/null
chmod 0755 "$MODDIR/bin/pluskeyd" "$MODDIR/service.sh" "$MODDIR/restart.sh" "$MODDIR/uninstall.sh" 2>/dev/null
chmod 0644 "$MODDIR/module.prop" "$MODDIR/skip_mount" 2>/dev/null
chmod 0644 "$MODDIR/webroot/icon.svg" "$MODDIR/webroot/index.html" 2>/dev/null
chmod 0600 /data/adb/OPlusKey/config.conf 2>/dev/null
